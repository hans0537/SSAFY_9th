...
// 이벤트 함수 만들기
// 이벤트 함수에 추가 파라미터 + 이베트 인자까지 사용하는 방법
const pickHand = (hand) => (e) => {
    ...
    // 인터벌 함수로 실행 setInterval(실행할 함수, 밀리초 단위)
    ...
    setTimeout(function() {
        ...
        let comPick = picks[Math.floor(ath.random() * 3)]
        ...
        modalContent.innerText = winner ? `승자는 ${winner} 입니다.` : "비겼ㅅ브니다."

        // modal의 display속성을 바꿔서 화면에 표시
        modal.style.display = "flex"
        
        // 다시 게임 진행 할 수 있도록 disable false 원상복구
...
document.querySelector('#scissors-button').addEventListener('click', pickHand('scissors'))
document.querySelector('#rock-button').addEventListener('click', pickHand('rock'))
document.querySelector('#paper-button').addEventListener('click', pickHand('paper'))